...
<div class="fixed top-[20px] left-[20px] bg-white p-4 rounded-sm shadow-md z-40 max-w-[350px] md:max-w-[450px] max-h-[700px] 2xl:max-h-[850px] overflow-auto {isVisible ? '' : 'hidden'}">
    ...
    {#each $recommendations as rec, i}
        <div class="flex flex-row gap-3 items-center mt-3 rounded">
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            ...
            {/if}
            <audio bind:this={audioPlayers[i]} src={rec.preview_url} volume="0"></audio>
        </div>
    {/each}
